...
class ContactPage extends React.Component {
  ...
  componentDidMount() {
    ...
    const elem = document.querySelector(".wakeTheBackend");
    elem.addEventListener("click", this.wakeTheBackend);
  }

  componentWillUnmount() {
    document
      .querySelector(".email")
      .removeEventListener("click", this.copyToClipboard);
  }
  ...
  wakeTheBackend = () => {
    if (this.state.hasWokenBackend === false) {
      const url = "https://devkavianbackend.herokuapp.com/";
      fetch(url)
        .then(() => {
          console.log("server is up");
          this.setState({ hasWokenBackend: true });
        })
        .catch(() => {
  ...
  render() {
    return (
      <div className="contact-wrapper">
        <TopMargin open={"open-other"}>CONTACT</TopMargin>
        ...
        <div className="body-cont">
          <div className={`form-wrapper ${this.state.msgSubmited}`}>
            <form
              ...
              className={`form-itself ${this.state.msgSubmited}`}
            >
              <FormInput
                wakeTheBackend
            ...
            {this.state.msgSubmited === "submited" ? (
              <div className={`thanks ${this.state.msgSubmited}`}>
                ...
                >
                  <Character characterDirection={"stand"} scale={3.5} />
                </div>
              </div>
            ) : null}
          </div>
